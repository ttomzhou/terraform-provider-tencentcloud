...
func testAccCheckAPIGatewayUsagePlanDestroy(s *terraform.State) error {
	for _, rs := range s.RootModule().Resources {
		...
		if !has {
			...
			return fmt.Errorf("delete apigateway usage plan %s fail, still on server", rs.Primary.ID)
		}
	}
	return nil
}
...
func testAccCheckAPIGatewayUsagePlanExists(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if has {
			...
			return fmt.Errorf("apigateway usage plan %s not found on server", rs.Primary.ID)
