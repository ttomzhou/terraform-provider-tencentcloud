				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"auth_type": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "API authentication type. Valid values: `SECRET` (key pair authentication),`NONE` (no authentication).",
						},
						"protocol": {
							Type:        schema.TypeString,
							Computed:    true,
...
func dataSourceTencentCloudAPIGatewayAPIsRead(d *schema.ResourceData, meta interface{}) error {
	...
	if outErr := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		apiSet, inErr = apiGatewayService.DescribeApisStatus(ctx, serviceId, apiName, apiId)
		if inErr != nil {
	...
	for _, apiKey := range apiSet {
		...
		if outErr = resource.Retry(readRetryTimeout, func() *resource.RetryError {
			info, has, inErr = apiGatewayService.DescribeApi(ctx, *apiKey.ServiceId, *apiKey.ApiId)
			if inErr != nil {
				return retryError(inErr, InternalError)
			}
			return nil
