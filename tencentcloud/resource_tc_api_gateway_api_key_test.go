		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet(testAPIGatewayAPIKeyResourceKey, "access_key_id"),
...
func testAccCheckAPIGatewayAPIKeyDestroy(s *terraform.State) error {
	for _, rs := range s.RootModule().Resources {
		...
		if !has {
			...
			return fmt.Errorf("delete apigateway api key %s fail, still on server", rs.Primary.ID)
		}
	}
	return nil
}
...
func testAccCheckAPIGatewayAPIKeyExists(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if has {
			...
			return fmt.Errorf("apigateway api key %s not found on server", rs.Primary.ID)
